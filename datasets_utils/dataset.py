...
import numpy as np
from datasets_utils.realesrgan import RealESRGAN_degradation
...
class PairedSROnlineTxtDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            # 将张量转换回PIL图像格式，范围[0, 1]
            # 首先将tensor转换为numpy，然后缩放到[0, 255]并转换为uint8
            # output_pil = Image.fromarray((output_t.permute(1, 2, 0).numpy() * 255).astype(np.uint8), mode='RGB')
            # img_pil = Image.fromarray((img_t.permute(1, 2, 0).numpy() * 255).astype(np.uint8), mode='RGB')
            # 对齐原来的数据范围，需要[0, 1]
            # # input images scaled to -1,1
            # img_t = F.normalize(img_t, mean=[0.5], std=[0.5])
            img_t = img_t * 2.0 - 1.0
            output_t = output_t * 2.0 - 1.0
            ...
            example["image"] = output_t
            # example["image"] = output_pil
            example["condition_0"] = img_t
            # example["condition_0"] = img_pil
            example["condition_type_0"] = "realesrgan"
            example["description"] = ""
