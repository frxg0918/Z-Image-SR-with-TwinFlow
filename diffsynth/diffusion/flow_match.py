...
class FlowMatchScheduler():
    ...
    def step(self, model_output, timestep, sample, to_final=False, **kwargs):
        ...
        if to_final or timestep_id + 1 >= len(self.timesteps):
            # sigma_ = torch.tensor(0, dtype=sigma.dtype, device=sigma.device)
            sigma_ = 0
        else:
            sigma_ = self.sigmas[timestep_id + 1]
        
        # sigma_ = sigma_.to(dtype=sample.dtype) # python标量没有to的操作
        # sigma = sigma.to(dtype=sample.dtype)
        # print(f"sigma: {sigma}, sigma_: {sigma_}, sigma dtype: {sigma.dtype}, sigma_ dtype: {sigma_.dtype}") # step()方法不会重新设置timesteps和sigmas float32
        # print("sample dtype: ", sample.dtype)
        # sample = sample.to(dtype=torch.float32)
        prev_sample = sample + model_output * (sigma_ - sigma)
        # print("prev_sample dtype:", prev_sample.dtype) # bfloat16
        return prev_sample
